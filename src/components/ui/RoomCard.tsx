...
const STATUS_CONFIG: Record<RoomStatus, { label: string; classes: string }> = {
  ...
  occupied:  { label: 'Ocupado',    classes: 'bg-red-100 text-red-700 border-red-200' },
...
export default function RoomCard({ room }: RoomCardProps) {
  ...
  const hasPhoto = Boolean(room.driveId)
  const waUrl = `https://wa.me/5561999999999?text=${encodeURIComponent(room.whatsappMsg)}`
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition-shadow">
      ...
      <div className="relative h-48 overflow-hidden bg-gray-100 flex-shrink-0">
        {hasPhoto ? (
          <img
            src={getDriveImageUrl(room.driveId)}
            alt={room.name}
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <a
          ...
          className="mt-auto flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white text-sm font-medium px-4 py-2.5 rounded-full transition-colors"
        >
